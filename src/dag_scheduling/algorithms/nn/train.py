...
from dag_scheduling.algorithms.nn.model import (
    score_tasks, CHROMOSOME_LEN, NN_FEATURE_INDICES,
)

# topology grid (same as MARL — 24 classes, fat=0.5 fixed)
            ...
            for j in _JUMP:
                for c in _CCR:
                    for _ in range(n_per_class):
                        dag = generate(
                            n=n, fat=0.5, regular=r, density=d,
                            jump=j, ccr=int(c * 10),
                        )
